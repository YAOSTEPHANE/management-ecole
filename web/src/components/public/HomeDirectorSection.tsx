...
const DEFAULT_DIRECTOR_NAME = "N'GUESSAN AMELA APOLLINE";
const DEFAULT_DIRECTOR_ROLE = 'Directrice des Études';
const DEFAULT_DIRECTOR_PHOTO = '/home/directrice-etudes.png';

function buildDirectorParagraphs(schoolName: string): string[] {
  return [
    'Chers parents d’élèves, Mesdames et Messieurs les enseignants, Chers élèves, membres du personnel éducatif et administratif,',
    `À l’aube de cette nouvelle année scolaire, la Direction de ${schoolName} adresse à l’ensemble de la communauté éducative ses salutations les plus chaleureuses ainsi que ses vœux de santé, de paix et de réussite.`,
    ...
    'La Direction rappelle aux parents d’élèves que l’école demeure le socle fondamental de la formation de l’enfant. Offrir une éducation de qualité, c’est préparer un citoyen responsable et compétent. Nous invitons chaque famille à accompagner le suivi scolaire et moral de son enfant tout au long de l’année.',
    'Aux enseignants et au personnel éducatif, la Direction renouvelle sa confiance dans les valeurs de rigueur, de professionnalisme et de responsabilité qui fondent notre mission.',
    'Aux élèves, nous adressons un appel à la discipline, au respect des règles, à l’assiduité et à la persévérance — le succès est le fruit du sérieux et de l’engagement personnel.',
...
export default function HomeDirectorSection() {
  ...
  const schoolName = contact.name;
  const directorName = branding.schoolPrincipal?.trim() || DEFAULT_DIRECTOR_NAME;
  const paragraphs = buildDirectorParagraphs(schoolName);
        ...
        <div className="overflow-hidden rounded-[2rem] border border-stone-200/90 bg-white shadow-[0_28px_56px_-24px_rgba(12,10,9,0.16)] ring-1 ring-tran-mustard-500/15">
          <div className="grid lg:grid-cols-12 lg:items-stretch">
            <div className="relative h-full border-b border-stone-200/80 lg:col-span-4 lg:border-b-0 lg:border-r">
              <div className="relative aspect-[3/4] w-full max-w-sm overflow-hidden sm:mx-auto lg:mx-0 lg:aspect-auto lg:h-full lg:max-w-none lg:min-h-[28rem]">
                {useCustomPhoto ? (
                  ...
                  <img
                    src={photoSrc}
                    alt={`Portrait de ${directorName}, ${DEFAULT_DIRECTOR_ROLE}`}
                ...
                <div className="absolute inset-x-0 bottom-0 z-10 p-6 text-white lg:p-8">
                  ...
                  <p className="mt-1 text-[11px] font-medium uppercase tracking-[0.12em] text-white/80 sm:text-xs">
                    {schoolName}
                  </p>
                </div>
              </div>
            ...
            <div className="flex flex-col justify-center p-6 sm:p-8 lg:col-span-8 lg:p-10 xl:p-12">
              <span className="home-ultra-label">À l’occasion de la rentrée scolaire</span>
              <h2
                id="director-message-title"
                className="home-ultra-section-title mt-5 text-2xl font-semibold text-stone-900 sm:text-3xl lg:text-[2rem] lg:leading-tight"
              >
                Mot de la Direction des Études
              </h2>
              ...
              <footer className="mt-8 border-t border-stone-200/80 pt-6">
                <p className="font-display text-sm font-semibold text-stone-900">{directorName}</p>
                <p className="mt-1 text-xs font-medium text-stone-600">{DEFAULT_DIRECTOR_ROLE}</p>
                <p className="mt-3 text-xs text-stone-500">{schoolName}</p>
              </footer>
            </div>
          </div>
        </div>
